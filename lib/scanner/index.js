...
class Scanner extends Emitter {
  ...
  onItemFound({ url, distance }) {
    ...
    // mozfestation hack
    if (url.indexOf('mozfstn') == -1) return;

    // TODO: only send if never seen before
    var mozfestURL = 'https://maker.ifttt.com/trigger/mozfest2016beacon/with/key/bN9E9VBcWZM0anX2b3K8zJ'
      + '?value1=' + this.installId + '&value2=' + url;

    fetch(mozfestURL, {
      method: 'GET'
    })
    // end mozfestation hack
