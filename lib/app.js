...
class App extends React.Component {
  constructor(props) {
    ...
    this.getInstallId().then((installId) => {
  ...
  setupNotificationListeners() {
    if (notification.launchedApp()) track.appLaunchFromNotification();
    notification.on('applaunch', () => track.appLaunchFromNotification());
    notification.on('dismiss', () => track.notificationDismiss());
  }

  getInstallId() {
    debug('get install id');

    return new Promise(function(resolve, reject) {
      AsyncStorage.getItem('app:installId')
        .then(value => {
          debug('got install id', value);
          if (value == null) {
            value = Math.random().toString(36).substr(2, 9);
            AsyncStorage.setItem('app:installId', String(value));
          }
          resolve(value);
        });
    });
  ...
  inflateUserFlags() {
    ...
    return Promise.all(Object.keys(flags).map(key => {
      return AsyncStorage.getItem(`user-prefs:${key}`)
        .then(value => {
   ...
   */
  onRefresh() {
    // mozfestation: disable clearing of any old items
    return;
    debug('on refresh');
    track.pullRefresh();
    this.stopScanning();
...
App.propTypes = {
  ...
  installId: React.PropTypes.string,
}
